  ...
  module Transaction
    ...
    class TransactionOutput
      ...
      def self.from_binary(data, offset = 0)
        raise ArgumentError, "truncated output: need 8 bytes for satoshis at offset #{offset}, got #{data.bytesize - offset}" if data.bytesize < offset + 8
        ...
        raise ArgumentError, "truncated output: need #{script_len} bytes for script at offset #{offset}, got #{data.bytesize - offset}" if data.bytesize < offset + script_len
