...
# Cross-SDK conformance tests for ProtoWallet.
#
...
RSpec.describe 'ProtoWallet cross-SDK conformance', :conformance do
  # -------------------------------------------------------------------------
  # BRC-3 Signature Compliance Vector
  # -------------------------------------------------------------------------
  describe 'BRC-3 signature compliance vector' do
    let(:wallet) { BSV::Wallet::ProtoWallet.new('anyone') }
  ...
  describe 'key derivation symmetry' do
    ...
    let(:user) { BSV::Wallet::ProtoWallet.new(user_key) }
    let(:counterparty_wallet) { BSV::Wallet::ProtoWallet.new(counterparty_key) }
  ...
  describe 'counterparty key linkage (BRC-69)' do
    ...
    let(:prover) { BSV::Wallet::ProtoWallet.new(prover_key) }
    let(:verifier) { BSV::Wallet::ProtoWallet.new(verifier_key) }
