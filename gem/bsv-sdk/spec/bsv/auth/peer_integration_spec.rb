...
RSpec.describe 'BSV::Auth::Peer integration' do # rubocop:disable RSpec/DescribeClass
  ...
  let(:alice_wallet)     { BSV::Wallet::Client.new(alice_key, storage: BSV::Wallet::MemoryStore.new) }
  let(:bob_wallet)       { BSV::Wallet::Client.new(bob_key, storage: BSV::Wallet::MemoryStore.new) }
  let(:carol_wallet)     { BSV::Wallet::Client.new(carol_key, storage: BSV::Wallet::MemoryStore.new) }
  ...
  describe 'Scenario 3: bidirectional certificate exchange' do
    ...
    let(:certifier_wallet2) { BSV::Wallet::Client.new(certifier_key2, storage: BSV::Wallet::MemoryStore.new) }
  ...
  describe 'Scenario 4: certificate validation failure — wrong subject' do
    ...
    let(:eve_wallet) { BSV::Wallet::Client.new(eve_key, storage: BSV::Wallet::MemoryStore.new) }
  ...
  describe 'Scenario 12: wrong certifier rejected' do
    ...
    let(:rogue_certifier_wallet) { BSV::Wallet::Client.new(rogue_certifier_key, storage: BSV::Wallet::MemoryStore.new) }
