...
RSpec.describe BSV::Network::ARC do
  ...
  describe '#broadcast_beef' do
    # Sample BEEF bytes — includes a null byte and high-bit characters to
    # exercise binary encoding. The actual content is irrelevant; ARC
    # validation is mocked.
    let(:beef_bytes) { "\x01\x00BEEF\x00\xFF\xFE".b }

    it 'returns a BroadcastResponse on success' do
      http = mock_http.new(200, success_body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      response = arc.broadcast_beef(beef_bytes)
    ...
    it 'sends Content-Type: application/octet-stream (not application/json)' do
      http = mock_http.new(200, success_body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      arc.broadcast_beef(beef_bytes)

      expect(http.last_request['Content-Type']).to eq('application/octet-stream')
    end

    it 'sends raw binary body (not JSON-wrapped)' do
      http = mock_http.new(200, success_body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      arc.broadcast_beef(beef_bytes)

      expect(http.last_request.body).to eq(beef_bytes)
      expect(http.last_request.body.encoding).to eq(Encoding::ASCII_8BIT)
    end
    ...
    it 'sends X-WaitFor header when wait_for is set' do
      http = mock_http.new(200, success_body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      arc.broadcast_beef(beef_bytes, wait_for: 'SEEN_ON_NETWORK')
    ...
    it 'sends X-SkipFeeValidation header when skip_fee_validation: true' do
      ...
      arc.broadcast_beef(beef_bytes, skip_fee_validation: true)
    ...
    it 'sends X-SkipScriptValidation header when skip_script_validation: true' do
      ...
      arc.broadcast_beef(beef_bytes, skip_script_validation: true)

      expect(http.last_request['X-SkipScriptValidation']).to eq('true')
    end

    it 'raises BroadcastError on non-2xx HTTP response' do
      body = { 'title' => 'Bad request', 'detail' => 'malformed BEEF', 'txid' => nil }.to_json
      http = mock_http.new(400, body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      expect { arc.broadcast_beef(beef_bytes) }.to raise_error(BSV::Network::BroadcastError) do |error|
        expect(error.message).to eq('malformed BEEF')
        expect(error.status_code).to eq(400)
      end
    end
    ...
    it 'raises BroadcastError when txStatus is DOUBLE_SPEND_ATTEMPTED' do
      ...
      expect { arc.broadcast_beef(beef_bytes) }.to raise_error(BSV::Network::BroadcastError, 'Double spend')
    end

    it 'treats SEEN_ON_NETWORK as success (idempotent re-broadcast)' do
      body = { 'txid' => 'abc123', 'txStatus' => 'SEEN_ON_NETWORK', 'title' => 'Already known' }.to_json
      http = mock_http.new(200, body)
      arc = described_class.new('https://arc.example.com', http_client: http)
    ...
    it 'treats MINED as success (idempotent re-broadcast)' do
    ...
    it 'raises BroadcastError on non-JSON response body' do
      http = mock_http.new(500, 'Internal Server Error')
      arc = described_class.new('https://arc.example.com', http_client: http)
    ...
    it 'raises BroadcastError on malformed 2xx (no txid)' do
      body = { 'txStatus' => 'SEEN_ON_NETWORK' }.to_json
      http = mock_http.new(200, body)
      arc = described_class.new('https://arc.example.com', http_client: http)

      expect { arc.broadcast_beef(beef_bytes) }
        .to raise_error(BSV::Network::BroadcastError, /malformed/)
    end

    it 'sends Authorization header when api_key is configured' do
      http = mock_http.new(200, success_body)
    ...
    it 'sends callback headers when configured' do
      http = mock_http.new(200, success_body)
      arc = described_class.new(
        'https://arc.example.com',
        callback_url: 'https://example.com/arc-cb',
